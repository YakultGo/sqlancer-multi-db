...
public class MySQLToStringVisitorTest {
    ...
    @Test
    void visitOracleAliasToString() {
        MySQLSchema.MySQLColumn c1 = new MySQLSchema.MySQLColumn("c1", MySQLSchema.MySQLDataType.INT, false, 0);
        MySQLSchema.MySQLTable t2 = new MySQLSchema.MySQLTable("t2", List.of(c1), Collections.emptyList(),
                MySQLSchema.MySQLTable.MySQLEngine.INNO_DB);
        c1.setTable(t2);
        MySQLColumnReference ref = new MySQLColumnReference(c1, null);
        assertEquals("t2.c1", MySQLVisitor.asString(new MySQLOracleAlias(ref, ref)));
        assertEquals("t2.c1", MySQLVisitor.asString(new MySQLOracleAlias(ref, null)));
    }

    @Test
    void visitSelectWithoutFromToString() {
        MySQLSelect s = new MySQLSelect();
        s.setFetchColumns(List.of(MySQLIntConstant.createIntConstant(1)));
        assertEquals("SELECT ALL 1 AS ref0", MySQLVisitor.asString(s));
    }
}
