...
public class MySQLDQEOracle extends DQEBase<MySQLGlobalState> implements TestOracle<MySQLGlobalState> {
    ...
    public String generateSelectStatement(AbstractTables<?, ?> tables, String tableName, String whereClauseStr) {
        ...
        // Disable ORDER BY/LIMIT variants for MySQL DQE.
        // They can introduce unstable row-access observations and lead to false positives.
        // Keep the core DQE consistency check on plain SELECT/UPDATE/DELETE statements.
    ...
    public void check() throws SQLException {
        // Reset per-check state to avoid leaking ORDER BY/LIMIT settings across iterations.
        generateOrderBy = false;
        generateLimit = false;
    ...
    public static class MySQLErrorCodeStrategy implements ErrorCodeStrategy {
        ...
        public Set<Integer> getUpdateSpecificErrorCodes() {
            ...
            // In MySQL, this can be WARNING in SELECT but ERROR in UPDATE/DELETE
            // depending on statement context and sql_mode, leading to DQE false positives.
